...
export function createFormActions<T>(featureName: string): FormActions<T> {
  ...
  const loadFailure = createAction(`[${featureName} API] Load ${featureName} Failure`, props<{ error: ErrorDto }>());
  ...
  const saveFailure = createAction(`[${featureName} API] Save ${featureName} Failure`, props<{ error: ErrorDto }>());
